defmodule Mix.Tasks.Ecsx.Gen.Component do
  ...
  Generates a new Component type for an ECSx application.
      ...
      $ mix ecsx.gen.component Color entity_id hue saturation lightness

  The first argument is the name of the Component type, followed by the data fields
  ...
  def run([component_type | _] = args) do
    create_component_file(args)
    inject_component_module_into_manager(component_type)
  end
  ...
  defp message_with_help(message) do
    ...
    mix ecsx.gen.component expects an component module name (in PascalCase).

    For example:

        mix ecsx.gen.component MyComponentType

    """
  end

  defp create_component_file([component_type | _opts]) do
